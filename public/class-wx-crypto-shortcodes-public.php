...
class Wx_Crypto_Shortcodes_Public {
	...
	public function ticker( $atts ) {
		$a           = shortcode_atts( array('referral_code' => ""), $atts );
		...
		if ( $ticker_data ) {
			...
			if($a['referral_code']){
				$affiliate_code = $this->get_affiliate_code_from_shortcode($a['referral_code']);
			}else{
				$affiliate_code = $this->get_affiliate_code();
			}
	...
	private function fetch_ticker( $symbol = '' ) {
		...
		if ( ! is_wp_error( $ticker_data ) ) {
			...
			set_site_transient( $transient_ticker_key, $ticker, WX_CRYPTO_TRANSIENT_CACHE );
		}
	...
	public function prepare_single_tick( $tick ) {
		...
		$tick['change_percentage'] = abs( $tick['change'] ) / $tick['openPrice'] * 100;

		$tick['change']            = $this->format_currency( $this->format_number( $tick['change'], 8, true ) );
		$tick['change_percentage'] = $this->format_number( $tick['change_percentage'], 2 ) . '%';
		$tick['logo']              = 'https://media.wazirx.com/media/' . $tick['baseAsset'] . '/84.png';
		return $tick;
	}
	...
	public function converter( $atts ) {
		...
		$converter_link = 'https://tools.wazirx.com/widgets/crypto-converter';
		ob_start();
	...
	public function return_converter( $atts ) {
		...
		$converter_link = 'https://tools.wazirx.com/widgets/crypto-roi';
		ob_start();
	...
	public function pp_calculator( $atts ) {
		...
		$converter_link = 'https://tools.wazirx.com/widgets/past-performance';
		ob_start();
	...
	public function charts_api() {
		register_rest_route(
			...
			array(
				'methods'             => 'GET',
				'callback'            => array( $this, 'charts_get_data' ),
	...
	public function verify_chart_interval( $interval ) {
		$allowed_intervals = array( '1H', '1D', '1W', '1M', '6M', '1Y', '10Y' );
	...
	public function get_affiliate_code_from_shortcode($code) {
			$code = esc_attr( $code );
			if ( $code ) {
				$referral_code = '?invite=' . $code;
				return $referral_code;
